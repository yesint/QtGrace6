...
int curformat = DEFAULT_PS_FORMAT;
...
int ps_grayscale = FALSE;
int ps_level2 = TRUE;
int docdata = DOCDATA_8BIT;

int ps_setup_offset_x = 0;
int ps_setup_offset_y = 0;

int ps_setup_grayscale = FALSE;
int ps_setup_level2 = TRUE;
int ps_setup_docdata = DOCDATA_8BIT;

int ps_setup_feed = MEDIA_FEED_AUTO;
int ps_setup_hwres = FALSE;

int eps_setup_grayscale = FALSE;
int eps_setup_level2 = TRUE;
int eps_setup_tight_bb = TRUE;
int eps_setup_docdata = DOCDATA_8BIT;
...
static int ps_initgraphics(int format)
{
    ...
    psfont_status = (int*)xmalloc(number_of_fonts()*sizeof(int));
...
void ps_fillarc(VPoint vp1, VPoint vp2, int a1, int a2, int mode)
{
    ...
    {
        //fprintf(prstream, "%.4f %.4f m\n", vpc.x, vpc.y);//original command without rotation
        fprintf(prstream, "%.4f %.4f m\n",vpc.x+vpc.x*ca-vpc.y*sa, vpc.y+vpc.x*sa+vpc.y*ca);
    }
