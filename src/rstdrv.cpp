...
void rst_setdrawbrush(void)
{
    ...
    if (rstlines > 1) {
        ...
        tmp_dash_array = (int *) xmalloc((scale*rst_dash_array_length + 1)*sizeof(int));
...
void rst_drawpolyline(VPoint *vps, int n, int mode)
{
    ...
    for (i = 0; i < n; i++)
    {
        /*Transformation --> rotate around centre*/
        x=p0.x+(vps[i].x-p0.x)*ca-(vps[i].y-p0.y)*sa;
        y=p0.y+(vps[i].y-p0.y)*ca+(vps[i].x-p0.x)*sa;
        vps[i].x=x;
        vps[i].y=y;
        /*End Transformation*/
...
int pnm_setup_format = DEFAULT_PNM_FORMAT;
int pnm_setup_rawbits = TRUE;
