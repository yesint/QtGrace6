...
int a=ba.indexOf(st.toUtf8());
int b=ba.indexOf(st2.toUtf8());
int c=ba.indexOf(QString(st).toUpper().toUtf8());
...
int res=0,start_comp_line=0,stop_compr_line=0,comr_method=0;
...
{
bool ok;
len=QInputDialog::getInt(0,QObject::tr("Expected Length"),QObject::tr("What would be the length (in Bytes) of the uncompressed data?"),1024,-2147483647,2147483647,1,&ok);
if (!ok) return;
}
else
{
