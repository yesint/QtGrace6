...
expr:	NUMBER {
	...
	| RAND
	{
        $$ = (((double)rand())/((double)RAND_MAX));//drand48();
	}
	| FUNC_I '(' iexpr ')'
	{
...
vexpr:
	...
	| RAND '(' nexpr ')'
	{
	    int i;
            $$ = &freelist[fcnt++];
	    $$->data = (double*)xmalloc($3*sizeof(double));
            if ($$->data == NULL) {
            ...
            for (i = 0; i < $$->length; i++) {
        $$->data[i] = (((double)rand())/((double)RAND_MAX));//drand48();
	    }
	}
	| REGNUM '(' selectset ')'
	{
	...
	| vexpr '^' vexpr
	{
	    ...
	    for (i = 0; i < $$->length; i++) {
            if ($1->data[i] < 0 && rint_2($3->data[i]) != $3->data[i]) {
	            yyerror("Negative value raised to non-integer power");
	            return 1;
	...
	| expr '^' vexpr
	{
	    ...
	    for (i = 0; i < $$->length; i++) {
            if ($1 < 0 && rint_2($3->data[i]) != $3->data[i]) {
	            yyerror("Negative value raised to non-integer power");
	            return 1;
...
regionset:
	...
	| REGNUM XY expr ',' expr
	{
	    rg[$1].x = (double*)xrealloc(rg[$1].x, (rg[$1].n + 1) * sizeof(double));
	    rg[$1].y = (double*)xrealloc(rg[$1].y, (rg[$1].n + 1) * sizeof(double));
	    rg[$1].x[rg[$1].n] = $3;
	    rg[$1].y[rg[$1].n] = $5;
	    rg[$1].n++;
	}
...
ticklabelattr:
	...
	| APPEND sexpr {
        ...
        strcpy(g[whichgraph].t[naxis]->orig_tl_appstr, $2);
	    xfree($2);
	}
	| PREPEND sexpr {
	    if (!is_valid_axis(whichgraph, naxis)) {
        ...
        strcpy(g[whichgraph].t[naxis]->orig_tl_prestr, $2);
	    xfree($2);
	}
	| ANGLE nexpr {
	    if (!is_valid_axis(whichgraph, naxis)) {
	...
	| nexpr ',' sexpr {
        ...
        g[whichgraph].t[naxis]->tloc[$1].orig_label =
                copy_string(g[whichgraph].t[naxis]->tloc[$1].orig_label, $3);
	    xfree($3);
	}
	| OFFSET AUTO {
	    if (!is_valid_axis(whichgraph, naxis)) {
	...
	| MINOR expr {
	    ...
	    if ($2 != 0.0) {
                g[whichgraph].t[naxis]->nminor = 
                            (int) rint_2(g[whichgraph].t[naxis]->tmajor / $2 - 1);
            } else {
